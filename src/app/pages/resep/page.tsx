...
import { Loader, ChevronRight, ChevronLeft, Clock, Users, ChefHat, Heart, AlertCircle, CheckCircle2, Sparkles, UtensilsCrossed, BookOpen, Trash2, Bookmark, Home } from 'lucide-react';
        ...
        <section className="px-4 lg:px-8 py-10 lg:py-20 text-center lg:text-left relative overflow-hidden" style={{ backgroundColor: '#EE6983' }}>
          ...
          <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8 items-center relative z-10">
            <div className="text-white order-2 lg:order-1">
              ...
              <h1 className="text-4xl lg:text-6xl font-black mb-6 leading-tight animate-fade-in-delay-1">
          ...
          <section className="px-4 lg:px-8 py-12 lg:py-20" style={{ backgroundColor: '#FFF5E4' }}>
            <div className="max-w-5xl mx-auto">
              ...
              <div className="bg-white rounded-[20px] shadow-2xl overflow-hidden relative min-h-[400px] transform rotate-1 hover:rotate-0 transition-transform duration-500">
                ...
                {/* Paper Content */}
                <div className="pl-12 md:pl-20 pr-4 md:pr-8 py-10 bg-[#fffdf0] min-h-full relative">
                  {/* Lined Paper Effect */}
                  ...
                  {/* Red Margin Line */}
                  <div className="absolute left-10 md:left-24 top-0 bottom-0 w-0.5 bg-red-200/50 z-0"></div>
        ...
        <div className="px-4 lg:px-8 py-8 lg:py-12">
          <div className="max-w-6xl mx-auto">
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 animate-fade-in gap-4">
              <div className="flex gap-3">
                ...
                {!viewingSavedRecipe && (
                  <button
                    ...
                    className="flex items-center gap-2 text-white font-bold bg-[#EE6983] hover:bg-[#D64D6B] transition-colors px-4 py-2 rounded-xl shadow-sm hover:shadow-md"
                  >
              ...
              {!viewingSavedRecipe && (
                <div className="text-sm font-bold text-white bg-[#EE6983] px-4 py-2 rounded-full shadow-lg self-end md:self-auto">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              <div className="lg:col-span-8 space-y-6 animate-fade-in-delay-1">
                {/* Title & Description - Organic Layout */}
                <div className="relative mb-8">
                  <div className="flex justify-between items-start mb-4">
                    <h1 className="text-4xl lg:text-6xl font-black text-[#B13455] leading-tight flex-1 pr-4 break-words drop-shadow-sm">
                      {currentRecipe.name}
                    </h1>
                    ...
                    {!viewingSavedRecipe && (
                      <button
                        ...
                        disabled={!!isSaved}
                        className={`p-4 rounded-full transition-all transform hover:scale-110 shadow-lg ${isSaved
                          ? 'bg-white/50 text-[#EE6983] cursor-default backdrop-blur-sm'
                  ...
                  <div className="bg-white/40 backdrop-blur-md p-6 rounded-[2rem] rounded-tl-none shadow-sm border border-white/50">
                    <p className="text-[#B13455]/80 text-lg leading-relaxed font-medium italic">
                  ...
                  <div className="flex flex-wrap gap-3 mt-6">
                    ...
                    <div className="flex items-center gap-2 bg-white/60 backdrop-blur-sm px-4 py-2 rounded-full border border-white/50 shadow-sm">
                      <ChefHat className="w-5 h-5 text-[#EE6983]" />
                      <span className="text-[#B13455] font-bold">
                ...
                {/* Ingredients & Instructions - Organic Flow */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  {/* Ingredients - Shopping List Style */}
                  <div className="relative">
                    <div className="absolute -top-3 -left-3 w-12 h-12 bg-[#FFF5E4] rounded-full flex items-center justify-center text-2xl shadow-sm z-10 border-2 border-[#EE6983]">
                      🥕
                    </div>
                    <div className="bg-white rounded-[2rem] rounded-tl-none p-8 shadow-lg border-2 border-[#FFE8F0] relative overflow-hidden">
                      <div className="absolute top-0 right-0 w-24 h-24 bg-[#FFE8F0] rounded-bl-[4rem] -mr-4 -mt-4 opacity-50"></div>

                      <h2 className="text-2xl font-black text-[#B13455] mb-6 pl-6">
                        Bahan-bahan
                      </h2>
                        ...
                        {currentRecipe.ingredients.map((ing, idx) => (
                          <li key={idx} className="flex items-start justify-between group">
                            <div className="flex items-start gap-3">
                              <div className="w-5 h-5 rounded-full border-2 border-[#EE6983]/30 mt-1 group-hover:bg-[#EE6983] group-hover:border-[#EE6983] transition-colors"></div>
                              <div>
                                <span className="font-bold text-gray-700 text-lg group-hover:text-[#B13455] transition-colors">{ing.item}</span>
                                {ing.nutrition && (
                                  <p className="text-xs text-[#EE6983] font-bold">{ing.nutrition}</p>
                                )}
                              </div>
                            </div>
                            <span className="font-black text-[#EE6983] bg-[#FFF5E4] px-3 py-1 rounded-lg text-sm whitespace-nowrap ml-2">
                              {ing.amount} {ing.unit}
                            </span>
                          </li>
                        ))}
                  ...
                  {/* Instructions - Timeline Style */}
                  <div className="relative">
                    <div className="absolute -top-3 -left-3 w-12 h-12 bg-[#FFF5E4] rounded-full flex items-center justify-center text-2xl shadow-sm z-10 border-2 border-[#EE6983]">
                      🍳
                    </div>
                    <div className="bg-white/60 backdrop-blur-md rounded-[2rem] rounded-tl-none p-8 shadow-lg border border-white relative">
                      <h2 className="text-2xl font-black text-[#B13455] mb-6 pl-6">
                        Cara Membuat
                      </h2>
                      <div className="space-y-8 relative">
                        {/* Timeline Line */}
                        <div className="absolute left-4 top-4 bottom-4 w-0.5 bg-[#EE6983]/20"></div>

                        {currentRecipe.instructions.map((step, idx) => (
                          <div key={idx} className="flex gap-6 relative group">
                            <div className="shrink-0 w-9 h-9 rounded-full bg-white border-4 border-[#EE6983] text-[#EE6983] flex items-center justify-center text-sm font-black shadow-sm z-10 group-hover:scale-110 transition-transform">
                              {idx + 1}
                            </div>
                            <p className="text-gray-700 text-lg leading-relaxed font-medium pt-1 group-hover:text-[#B13455] transition-colors">{step}</p>
                          </div>
                        ))}
                      </div>
                    </div>
              ...
              <div className="lg:col-span-4 space-y-8 animate-fade-in-delay-2">
                {/* Nutrition Benefits - Organic Blob Shape */}
                <div className="bg-[#EE6983] rounded-[3rem] rounded-tr-none p-8 text-white shadow-xl relative overflow-hidden transform hover:scale-105 transition-transform duration-300">
                  ...
                  <ul className="space-y-4 relative z-10">
                    {currentRecipe.nutritionBenefits.map((benefit, idx) => (
                      <li key={idx} className="flex gap-3 items-start bg-white/10 p-4 rounded-2xl backdrop-blur-sm hover:bg-white/20 transition-colors border border-white/10">
                ...
                {/* Tips - Sticky Note Style */}
                <div className="bg-[#FFF5E4] rounded-[2rem] rounded-br-none p-8 border-2 border-[#FFE8F0] shadow-lg relative transform rotate-1 hover:rotate-0 transition-transform duration-300">
                  <div className="absolute -top-3 left-1/2 transform -translate-x-1/2 w-32 h-6 bg-[#EE6983]/20 rounded-full blur-sm"></div>

                  <h2 className="text-2xl font-black text-[#B13455] mb-4 flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-white flex items-center justify-center shadow-md text-xl">
                      💡
                    </div>
                    Tips Koki
                  </h2>
                  <p className="text-[#B13455] text-lg leading-relaxed italic font-medium bg-white/50 p-4 rounded-xl border border-[#B13455]/10">
                    "{currentRecipe.tips}"
                  </p>
                </div>
